<template>
  <div :class="cn('relative inline-block', props.class)">
    ...
    <!-- Preview -->
    <div v-if="isVisible" ref="preview" class="pointer-events-none absolute z-50"
      :style="previewStyle">
      <div class="overflow-hidden rounded-xl shadow-xl"
        ...
        <div
          ...
          <!-- <img :src="previewSrc" :width="width" :height="height"
            class="size-full rounded-lg object-cover" :style="imageStyle"
            alt="preview" @load="handleImageLoad" /> -->
          <img :src="previewSrc" :width="width" :height="height"
            class="size-full rounded-lg object-cover" :style="imageStyle"
            alt="preview" />
        </div>
...
import { ref, type CSSProperties, computed, reactive } from "vue";
...
import { ClassWatcher } from '@/lib/utils'

const setPreviewImage = () => {

  if (props.url.endsWith("2020")) previewSrc = "PersonalSite2020.png"
  if (props.url.endsWith("2021")) previewSrc = "PersonalSite2021.png"
  if (props.url.endsWith("2022")) {
    if (document.getElementsByTagName('html')[0].classList.contains('dark')) {
      previewSrc = "PersonalSite2022dark.png"
    } else {
      previewSrc = "PersonalSite2022.png"
    }
  }
  if (props.url.endsWith("2023")) {
    if (document.getElementsByTagName('html')[0].classList.contains('dark')) {
      previewSrc = "PersonalSite2023dark.png"
    } else {
      previewSrc = "PersonalSite2023.png"
    }
  }
}


let classWatcher = new ClassWatcher(document.getElementsByTagName('html')[0], 'trigger', setPreviewImage, setPreviewImage)
